  ...
  void setup() {
    ...
    s01.attach(5); s02.attach(35); s03.attach(7); 
  ...
  void handleInputCommands() {
    ...
    if (m == 16) tarP[0]+=5; if (m == 17) tarP[0]-=5;
    if (m == 19) tarP[1]+=5; if (m == 18) tarP[1]-=5;
    if (m == 20) tarP[2]+=5; if (m == 21) tarP[2]-=5;
    if (m == 23) tarP[3]+=5; if (m == 22) tarP[3]-=5;
    if (m == 25) tarP[4]+=5; if (m == 24) tarP[4]-=5;
    if (m == 26) tarP[5]+=5; if (m == 27) tarP[5]-=5;
  ...
  void smoothMove() {
    bool moved = false;
    if (curP[0] != tarP[0]) { curP[0] += (tarP[0] > curP[0]) ? 1 : -1; s01.write(curP[0]); moved = true; }
    if (curP[1] != tarP[1]) { curP[1] += (tarP[1] > curP[1]) ? 1 : -1; s02.write(curP[1]); moved = true; }
    if (curP[2] != tarP[2]) { curP[2] += (tarP[2] > curP[2]) ? 1 : -1; s03.write(curP[2]); moved = true; }
    if (curP[3] != tarP[3]) { curP[3] += (tarP[3] > curP[3]) ? 1 : -1; s04.write(curP[3]); moved = true; }
    if (curP[4] != tarP[4]) { curP[4] += (tarP[4] > curP[4]) ? 1 : -1; s05.write(curP[4]); moved = true; }
    if (curP[5] != tarP[5]) { curP[5] += (tarP[5] > curP[5]) ? 1 : -1; s06.write(curP[5]); moved = true; }

    if (moved) {
      Serial.print("S:");
      Serial.print(curP[0]); Serial.print(",");
      Serial.print(curP[1]); Serial.print(",");
      Serial.print(curP[2]); Serial.print(",");
      Serial.print(curP[3]); Serial.print(",");
      Serial.print(curP[4]); Serial.print(",");
  ...
  void setTargets(int t0, int t1, int t2, int t3, int t4, int t5) {
    tarP[0] = t0; tarP[1] = t1; tarP[2] = t2; 
    tarP[3] = t3; tarP[4] = t4; tarP[5] = t5;
  }
